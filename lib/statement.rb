...
module Statement
  ...
  class Link
    ...
    def self.from_rss(url)
      ...
      links.map{|link| {:source => url, :url => link.xpath('link').text, :title => link.xpath('title').text, :date => link.xpath('pubDate').empty? ? nil: Date.parse(link.xpath('pubDate').text), :domain => URI.parse(link.xpath('link').text).host }}
    end
