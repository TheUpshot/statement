...
module Statement
  class Scraper
    ...
    def self.member_methods
      [:capuano, :cold_fusion, :conaway, :chabot, :susandavis, :freshman_senators, :klobuchar, :lujan, :billnelson, :lautenberg, :crapo, :coburn, :boxer, :vitter, :donnelly, :inhofe, :reid, :palazzo, :roe, :document_query, :swalwell, :fischer, :clark, :edwards, :culberson_chabot, :barton, :wolf, :welch]
    end
    ...
    def self.member_scrapers
      ...
      results = [capuano, cold_fusion(year, 0), conaway, chabot, susandavis, klobuchar, lujan, palazzo(page=1), roe(page=1), billnelson(year=year), 
        ...
        vitter(year=year), inhofe(year=year), reid, fischer, clark(year=year), edwards, culberson_chabot(page=1), barton, wolf, welch].flatten
    ...
    def self.backfill_from_scrapers
      results = [cold_fusion(2012, 0), cold_fusion(2011, 0), cold_fusion(2010, 0), billnelson(year=2012), document_query(page=3), 
        ...
        boxer(start=31), boxer(start=41), vitter(year=2012), vitter(year=2011), swalwell(page=2), swalwell(page=3), clark(year=2013), culberson_chabot(page=2),
        wolf(page=1)].flatten
      Utils.remove_generic_urls!(results)
    end
    ...
    def self.culberson_chabot(page=1)
      results = []
      domains = [{'culberson.house.gov' => 2573}, {'chabot.house.gov' => 2508}]
      domains.each do |domain|
    ...
    def self.wolf(page=0)
      results = []
      domain = 'wolf.house.gov'
      url = "http://wolf.house.gov/media-center/press-releases?page=#{page}"
      doc = open_html(url)
      return if doc.nil?
      dates = doc.xpath('//span[@class="field-content"]').map {|s| s.text if s.text.strip.include?("2014")}.compact!
